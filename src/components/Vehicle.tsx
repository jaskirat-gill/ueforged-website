...
const Wheels: FC<WheelsProps> = memo(
  ({
    ...
    const tire_front_diameter = ((converted_rim_front_width * (tire_aspectRatio / 100)) * 2) + rim_front_diameter
    const tire_rear_diameter = ((converted_rim_rear_width * (tire_aspectRatio / 100)) * 2) + rim_rear_diameter
    ...
    const frontTireGeometry = useMemo(() => {
      ...
      const frontWidthScale =
        wheelFrontWidth / vehicleConfigs.wheels.tires[tire].width;
    ...
    const rearTireGeometry = useMemo(() => {
      ...
      const rearWidthScale =
        wheelRearWidth / vehicleConfigs.wheels.tires[tire].width;
    ...
    const odFrontScale = useMemo(
      () =>
        ...
        vehicleConfigs.wheels.rims[rim].od,
    ...
    // Calculate rim width.
    const widthFrontScale = useMemo(() => (rim_front_width * 2.54) / 100 / vehicleConfigs.wheels.rims[rim].width, [rim, rim_front_width])
    const widthRearScale = useMemo(() => (rim_rear_width * 2.54) / 100 / vehicleConfigs.wheels.rims[rim].width, [rim, rim_rear_width])
...
const scaleTireGeometry = (
  ...
  const tireID = vehicleConfigs.wheels.tires[label].id
    ? vehicleConfigs.wheels.tires[label].id / 2
    : 0;
...
const Vehicle: FC<VehicleProps> = ({ currentVehicle, setVehicle }) => {
  ...
  const axleHeight = useMemo(
    () => (((((rim_rear_width / 25.4) * (tire_aspectRatio / 100)) * 2) + rim_rear_diameter) * 2.54) / 100 / 2,
    [tire_aspectRatio]
  );
